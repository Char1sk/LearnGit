...
不同远程库的名字不能重复（origin为默认远程库名）

删除已关联的远程库
> git remote rm _name_

关联远程库
> git remote add _new-name_ _url_
...
在工作区的根目录创建gitignore文件（本身要被放入版本库管理），然后把要忽略的文件名填进去，git就会自动忽略这些文件。
...
设置别名（全局）（香爆）
> git config --global alias._ref-name_ "_true-name_"

当前仓库的配置文件存放在.git/config文件，别名在[alias]一栏。

用户的配置文件在用户主目录下./gitconfig中，别面在[alias]一栏。
